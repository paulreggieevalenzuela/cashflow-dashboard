import type { StaffBonusRow } from "@/lib/db/transactions";
...
export function StaffLeaderboardTable({ rows }: { rows: StaffBonusRow[] }) {
  if (rows.length === 0) {
    return (
      <div className="rounded-lg border border-dashed border-zinc-300 px-6 py-10 text-center text-sm text-zinc-500 dark:border-zinc-700 dark:text-zinc-400">
        No bonus rates set yet, or no transactions recorded by a staff
        account with a rate for that procedure. Set rates on the
        Commissions page.
      </div>
    );
  }
    ...
    <div className="overflow-x-auto rounded-lg border border-zinc-200 dark:border-zinc-800">
      <table className="w-full min-w-[560px] text-left text-sm">
        ...
        <tbody className="divide-y divide-zinc-100 dark:divide-zinc-800">
          {rows.map((row, index) => (
            <tr key={row.staffUserId} className="text-zinc-700 dark:text-zinc-300">
              ...
              <td className="whitespace-nowrap px-4 py-3 text-right tabular-nums">
                {currencyFormatter.format(row.bonusAmount)}
              </td>
              <td className="whitespace-nowrap px-4 py-3 text-right tabular-nums">
